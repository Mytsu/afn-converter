...
class Automato(object):
    ...
    #Função que verifica se o estado tem ligação saindo dele para ele mesmo, caso não tenha retornará uma string "None"
    ...
    #Caso os três valores sejam "None" ele retornará apenas um "None"
    #Caso tenha pelo menos um símbolo diferente de "None" ele que será retornado
    def simbolos_transicao(self, vetor_simbolos):
        simbolo = ''
        for i in range(len(vetor_simbolos)):
            if vetor_simbolos[i] != 'None':
                simbolo = simbolo + vetor_simbolos[i]   #Concatenando os símbolos diferentes de "None"

        if simbolo == '':   #Condição que verifica se a VAR símbolos tem algo ou vazia, caso esteja vazia retornará "None"
            return "None"
        else:
    ...
    def remove_laco_interno(self, nome_no):
        ...
        for i in range(len(self.automato[posicao_no].transicoes)):
            if self.automato[posicao_no].transicoes[i][0] != "None":    #Caso o símbolo seja diferente de apenas "None", será concatenado aos outros símbolos
    ...
    def deleta_estado(self, deleta_nome):
        if self.verifica_laco_interno(deleta_nome) != "None":   #Já tratando o caso de laço interno no estado, fecho de Klen
        ...
        for i in range(len(vertices_que_vao_ao_no)):
            for j in range(len(lista_transicoes_no_deletado)):
                ...
                if deleta_nome != destino:
                    ...
                    if pos_ligacao != int(-1) and aux != "None":    #Caso exista e o simbolo de transião seja "None" é feita uma concatenação
